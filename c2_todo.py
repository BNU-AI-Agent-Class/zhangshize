...
- 完成：{"done": "总结"}
接到多步任务时，先用 todo_write 列出计划；每完成一步就用 todo_write 更新勾选状态。
不要说「我无法访问」「环境不在本地」——你指定工具调用，脚本会帮你执行。"""  # 9. 要求"先列计划再动手"

client = OpenAI(api_key=API_KEY, base_url=BASE_URL)                # 10. DeepSeek 客户端
