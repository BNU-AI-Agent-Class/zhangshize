# s5_agent.py —— 加专业 Skill
# 目标：同一个 Agent，换知识包。系统提示词 = agent.md + skill.md。
# 示例 skill.md 是"用户研究专家"，可替换为导师、教练、产品经理等其它领域。
...
def terminal_chat():
    ...
    print_message("系统", "已连接 Kimi 用户研究专家。输入消息直接对话，输入 /new 开启新对话，/quit 退出。")
